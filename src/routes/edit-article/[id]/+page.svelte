<script>
  ...
  export let data;
  const post = data;
  let title=post.data.title,category=post.data.category,content=post.data.content;
  ...
  const edit = async (opt) => {
    ...
    requestData.status = opt
    let path = "/all-post/published"
    if (opt == "draft") {
      path = "/all-post/drafts"
    }
    let res = await postData(`http://127.0.0.1:1234/article/${post.data.id}`, requestData)
...
<div class="container mx-auto max-w-4xl mt-24 border-2 border-black rounded-lg p-4 shadow-lg bg-gray-700">
  <form>
    ...
    <div class="flex justify-center gap-x-4">
      <button on:click={() => edit("publish")} class="text-white focus:ring-4 focus:outline-none font-medium rounded-lg text-sm w-full px-5 py-2.5 text-center bg-green-500 hover:bg-green-700 focus:ring-green-800">Publish</button>
      <button on:click={() => edit("draft")} class="text-white focus:ring-4 focus:outline-none font-medium rounded-lg text-sm w-full px-5 py-2.5 text-center bg-yellow-500 hover:bg-yellow-700 focus:ring-yellow-800">Draft</button>
    </div>
  </form>
</div>
